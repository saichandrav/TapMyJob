...
export default function JobFeed() {
  ...
  return (
    <>
      <div className="grid min-h-[calc(100vh-7rem)] gap-6 md:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[280px_minmax(0,1fr)_380px]">
      ...
      <main className="min-w-0 space-y-5">
        ...
        <aside className="hidden rounded-3xl border border-border bg-surface/95 p-5 shadow-card-glow xl:sticky xl:top-20 xl:block xl:h-[calc(100vh-6rem)] xl:overflow-y-auto">
        {selectedJobData ? (
          <div className="space-y-5">
            <div className="flex items-start gap-4">
              <CompanyMark company={selectedJobData.company} logoUrl={selectedJobData.logoUrl} />
              <div className="min-w-0 flex-1">
                <div className="font-display text-xl font-semibold leading-tight text-text-primary">{selectedJobData.title}</div>
                <div className="mt-1 text-sm text-text-muted">{selectedJobData.company}</div>
            ...
            <div className="space-y-2 rounded-3xl border border-border bg-card p-4">
              <div className="font-mono text-sm text-secondary">{formatSalary(selectedJobData)}</div>
              <div className="text-xs text-text-muted">{selectedJobData.experience} · {formatPostedTime(selectedJobData)}</div>
            </div>

            <div className="flex gap-2">
              <button className="flex-1 rounded-2xl border border-primary/30 bg-primary px-4 py-3 text-sm font-medium text-background transition hover:shadow-accent-glow">
                Apply
              </button>
              <button
                onClick={() => toggleSavedJob(selectedJobData.id)}
                className="rounded-2xl border border-border bg-card px-4 py-3 text-sm text-text-primary transition hover:border-primary/40"
              >
                Save
              </button>
            </div>
            ...
            <a
              href={selectedJobData.sourceUrl}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 text-sm text-secondary transition hover:opacity-80"
            >
              View on {platformLabelMap[selectedJobData.platformId] ?? selectedJobData.platformId} <ExternalLink className="h-4 w-4" />
            </a>

            <DetailSection title="Job description">
              <div className="max-h-40 overflow-y-auto pr-2 text-sm leading-6 text-text-muted">
            ...
            <DetailSection title="Skills">
              <div className="flex flex-wrap gap-2">
                {(selectedJobData.skills ?? []).map((skill) => (
                  <span key={skill} className="rounded-full border border-border bg-background px-3 py-1.5 font-mono text-[11px] text-text-muted">
                    {skill}
                  </span>
                ))}
              </div>
            ...
            <DetailSection title="Company info">
              <div className="space-y-2 text-sm text-text-muted">
                <div>Category: {selectedJobData.category}</div>
                <div>Source: {platformLabelMap[selectedJobData.platformId] ?? selectedJobData.platformId}</div>
                <div>
                  Hiring for {selectedJobData.company} with active openings across frontend, backend, and data roles.
                </div>
              </div>
            </DetailSection>
            ...
            <DetailSection title="Similar jobs">
              <div className="space-y-3">
                {jobs
                  .filter((job) => job.id !== selectedJobData.id && job.category === selectedJobData.category)
                  .slice(0, 3)
                  .map((job) => (
                    <button
                      key={job.id}
          ...
          <div className="rounded-3xl border border-dashed border-border bg-card/50 p-8 text-sm text-text-muted">
      ...
      <div
        className={`fixed inset-0 z-40 bg-black/65 backdrop-blur-sm transition-opacity duration-200 xl:hidden ${
          detailOpen && selectedJobData ? 'opacity-100' : 'pointer-events-none opacity-0'
        }`}
        onClick={() => setDetailOpen(false)}
      />

      <aside
        className={`fixed inset-y-0 right-0 z-50 w-full max-w-95 border-l border-border bg-surface/98 p-5 shadow-2xl backdrop-blur-xl transition-transform duration-300 xl:hidden ${
          detailOpen && selectedJobData ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        {selectedJobData ? (
